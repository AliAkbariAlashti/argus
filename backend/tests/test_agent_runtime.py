...
def test_agent_runs_multiple_tools_then_answers(monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_complete", lambda messages, tools=None: next(replies))
    calls = []
...
def test_agent_stops_at_step_limit(monkeypatch):
    ...
    monkeypatch.setattr(runtime, "_complete", lambda messages, tools=None: {"role": "assistant", "tool_calls": [{"id": "x", "function": {"name": "unknown", "arguments": "{}"}}]})
