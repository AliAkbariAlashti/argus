...
(async () => {
  ...
  try {
    ...
    assert(await page.locator('#view-chat .chat-strip-col').isHidden());
    assert(await page.locator('#view-chat .viewer').isHidden());
    assert(await page.locator('#new-chat').isVisible());
    await page.locator('#rail-collapse').click();
    assert(await page.locator('.app').evaluate(node => node.classList.contains('rail-collapsed')));
    await page.locator('#sidebar-toggle').click();
    assert(!await page.locator('.app').evaluate(node => node.classList.contains('rail-collapsed')));
    await page.locator('.rail-btn[data-view="dashboard"]').click();
    await page.waitForSelector('.fleet-tile');
    await page.locator('.fleet-tile').first().click();
    assert.equal(await page.locator('#viewer-stream').getAttribute('src'), null);
    ...
    assert(!await page.locator('#evidence-dialog').isVisible());
    await page.route('**/api/chat/history', route => route.request().method() === 'DELETE' ? route.fulfill({contentType:'application/json', body:'{"deleted":2}'}) : route.continue());
    await page.route('**/api/chat/sessions', route => route.request().method() === 'POST' ? route.fulfill({contentType:'application/json', body:JSON.stringify({id:'test-session',title:'New chat',created_at:new Date().toISOString(),updated_at:new Date().toISOString()})}) : route.continue());
    await page.locator('#new-chat').click();
    await page.getByText('What should Argus handle?').waitFor();
